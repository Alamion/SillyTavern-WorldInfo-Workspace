...
- [X] T012 Add optional `assistant?: AssistantSettings` to `WorkspaceSettings` (next to `sortMode`) in `src/core/state/schema.ts` with `getAssistantSettings(state)` and repair in `migrate`, plus a pure immutable `setAssistantSettings(state, patch)` op in the new file `src/core/assistant/settingsOps.ts` to pass T011
...
- [X] T025 [US3] Implement `src/core/assistant/scope.ts` to pass T024
...
- [X] T027 [P] [US4] Extend `tests/integration/assistant-controller.test.ts`: `FakeLlm.failTimes(2, rate-limit).then(reply)` → message goes `failed` → `retry-wait` (snapshot exposes `retryAt`) → retried automatically twice with 10 s / 30 s (fake timers) → `received`; a third rate limit stays `failed` with manual `retry()`; `stop()` during `retry-wait` cancels the countdown; `retryNow()` skips the wait; `provider` failure → no auto retry, `retry()` re-sends the SAME `requestMessages`; `timeout` → failed, retryable; the workspace store state is reference-equal before and after every failure path; a hanging assistant request does not block a parallel `send` in another conversation
- [X] T028 [US4] Implement retry state machine, `retry`, `retryNow`, cancel-on-stop and `requestMessages` storage in `ContextSnapshot` in `src/adapters/assistantController.ts` to pass T027 (retry policy from `src/core/assistant/failures.ts`)
- [X] T029 [P] [US4] Create `src/ui/assistant/FailureCard.tsx`: readable reason, countdown "Retrying in 30 s" with [Cancel] / [Retry now] for `retry-wait`, [Retry] for retryable failures, collapsible provider detail; rendered by `ConversationView.tsx` under the failed assistant message; the user message stays visible
...
- [X] T031 [P] [US1] Write failing unit tests `tests/unit/assistant-handles.test.ts`: `buildHandleMap(state, scope)` assigns `f`/`e`/`i` + running numbers in tree order, stable for the same state, reverse lookup; unknown handle → `undefined`
...
- [X] T036 [P] [US1] Extend `tests/contract/hooks.test.ts` with the `wi-workspace:assistant-applied` payload contract per `contracts/hooks.md` (fields and types; no prompt text or profile data; nothing emitted for a failed apply with zero applied operations) — constitution VI: lands in the same change as the emitter T042
- [X] T037 [US1] Implement `src/core/assistant/protocol.ts` to pass T030: `createProtocolParser()` with `feed(accumulated: string)` and `finish()` returning `{ prose: ProseSegment[]; blocks: ParsedBlock[]; unparsed; reasoning; references: string[] }`, plus exported model-facing text constants `PROPOSE_PROTOCOL_TEXT` and `DISCUSS_PROTOCOL_TEXT` copied verbatim from the contract
- [X] T038 [P] [US1] Implement `src/core/assistant/handles.ts` to pass T031
- [X] T039 [US1] Implement `src/core/assistant/prompts.ts` (default instructions text focused on evocative lore writing for small models; decision-note formatter stub used in US5) and `src/core/assistant/context.ts` to pass T032
- [X] T040 [US1] Implement `src/core/assistant/validate.ts` to pass T033 (create_entry, edit_entry; other op types return `invalid: "not supported yet"` until US2)
- [X] T041 [P] [US1] Implement `src/core/assistant/rules.ts` to pass T034
- [X] T042 [US1] Implement `src/adapters/assistantApply.ts` (entries part) to pass T035: `applyProposals({ store, sync, emit }, batch, proposalIds)` validates staleness, applies in reply order via `workspaceActions.applyTreeChange` with `createChild` + `commitEntryField` for each set field (title through `renameNode`/`comment`), records `AppliedItem`s with inverses (`delete-created`, `restore-fields`) and `afterUpdatedAt`, stops on the first failure recording `failed` (FR-028), emits the applied event
...
- [X] T044 [P] [US1] Create `src/ui/assistant/ProposalCard.tsx`: op icon (reuse the mock's icon map), summary with tree path of target/parent, badges (destructive, duplicate → "Similar to <name>", stale, invalid + reason), Accept / Deny / Edit / Diff / Feedback (inline input calling `feedback(messageSeq, text, proposalId)`) buttons with disabled states per decision; create_entry preview shows title, keys, fields list and content rendered through the preview renderer
- [X] T045 [P] [US1] Create `src/ui/assistant/ProposalEditor.tsx`: modal editing title, keys (comma list), secondary keys, content, and scalar fields present in the proposal; saves via controller `editProposal`
- [X] T046 [P] [US1] Create `src/ui/assistant/ProposalDiff.tsx`: per-field before/after using `src/ui/DiffView.tsx` (content as line diff, scalar fields as one-line rows), also used for the stale refresh
- [X] T047 [US1] Create `src/ui/assistant/BatchBar.tsx` (Accept all pending / Deny all pending / Feedback… input) and `src/ui/assistant/ReplyNotices.tsx`: context notice ("Sent: … Omitted: …" from `ContextSnapshot.omitted`); truncated notice with [Continue] [Regenerate]; [Regenerate] also in the assistant message footer; collapsed "Previous version" when `previousText` is set; broken-blocks notice "N operation blocks could not be used." with [Show broken blocks] (collapsed list of raw excerpts + reasons) and [Regenerate with the same context], plus [Ask to fix] when zero proposals are valid; wire proposals, bar and notices into `ConversationView.tsx`
- [X] T048 [US1] Render `[[handle]]` references in assistant prose as item links in `src/ui/assistant/ConversationView.tsx` (resolve through the message's `ContextSnapshot.handles`; click selects the node and opens it in the editor via a callback from `src/ui/WorkspaceApp.tsx`; missing node → `notifyWarning` from `src/adapters/logger.ts`: "This item no longer exists")
...
- [X] T049 [P] [US2] Extend `tests/unit/assistant-validate.test.ts`: create_folder, rename, move, delete per the required-attributes table; move into own subtree / no-op move / delete or rename of the workspace root → invalid; ref used as move parent must be a folder creation
...
- [X] T053 [P] [US2] Extend `tests/contract/hooks.test.ts` with the `wi-workspace:assistant-undone` payload contract per `contracts/hooks.md` (`reverted`/`skipped` node id arrays) — lands in the same change as the emitter T059
...
- [X] T055 [US2] Extract the delete flow from `src/ui/WorkspaceApp.tsx` (confirmation label with linked-files note, `collectEntityDeletions`, `sync.recordEntityDeletions`, root-book keep/delete question, `bulkDeleteNodes` via `applyTreeChange`) into `deleteNodes(deps, ids, { confirm, preconfirmed?, rootBooks? })` plus `describeDeletion(state, ids)` (affected items incl. descendants, books, linked files) in `src/adapters/workspaceActions.ts`; make `WorkspaceApp.tsx` call it with no behavior change (T054 green)
- [X] T056 [US2] Extend `src/core/assistant/validate.ts` for create_folder/rename/move/delete to pass T049, and set `destructive` for deletions
- [X] T057 [P] [US2] Implement `src/core/assistant/plan.ts` to pass T050
- [X] T058 [P] [US2] Implement `src/core/assistant/undo.ts` to pass T051
- [X] T059 [US2] Extend `src/adapters/assistantApply.ts` to pass T052: ordering via `plan.ts`; ops via `createChild('folder')`, `renameNode`, `moveNode` (effects `structure: true`), `deleteNodes` from T055 (called with `preconfirmed: true` after the assistant's own confirmation); inverses `restore-name`, `restore-position` (parent + index captured before the move), `reinsert` (deep subtree snapshot + index); `undoBatch(deps, conversationId, batch)` executing `planUndo` steps through the same tree ops (re-insert through a new pure op `insertSubtree(state, parentId, index, subtree)` added to `src/core/tree/operations.ts` with a unit test in `tests/unit/tree-operations.test.ts`) and emitting the undone event
- [X] T060 [US2] Controller support in `src/adapters/assistantController.ts`: blocked recomputation after every decision, `acceptAll` via `acceptAllSelection`, `confirmDestructive(messageSeq, proposalId)` (shows `describeDeletion` text for deletions or the diff summary for destructive edits through `src/adapters/popups.ts`), `undoBatch(messageSeq, batchId)` with result toast via `src/adapters/logger.ts` `notifyInfo`/`notifyWarning`: "Reverted N changes; M skipped (edited since)"
- [X] T061 [US2] UI: blocked and destructive states in `src/ui/assistant/ProposalCard.tsx` ("Blocked: needs <summary>", "Confirm delete…" / "Confirm edit…" button instead of Accept), move/rename previews (source → destination path, old → new name), deletion preview listing affected items; after Accept all scroll the first remaining destructive card into view; [Undo batch] per `AppliedBatch` with skipped details in `src/ui/assistant/BatchBar.tsx`
...
- [X] T062 [P] [US5] Write failing unit tests `tests/unit/assistant-decisions.test.ts` for `decisionNote(batch)` in `src/core/assistant/prompts.ts`: compact English note listing accepted/denied/applied/undone proposals by op + title, "(do not propose again unless asked)" for denied; empty batch → no note
- [X] T063 [P] [US5] Extend `tests/integration/assistant-controller.test.ts`: controller restart over the same `MemoryConversationStore` restores conversations, messages, batches with decisions and applied batches; pending proposals of a restored batch are re-validated for staleness on accept; history sent in a later request includes `note` messages after decided batches; `renameConversation` (non-empty), `deleteConversation` (removes messages, selects the most recent remaining or none); conversation titles default to the first 60 chars of the first request; per-conversation `context` and `mode` persist
- [X] T064 [US5] Implement `decisionNote` in `src/core/assistant/prompts.ts` and history assembly with notes in `src/core/assistant/context.ts` (notes count toward the budget, older turns trimmed first per R6) to pass T062
- [X] T065 [US5] Implement restore on controller init, rename, delete, title defaulting, note messages after decisions/undo, and store flushes on decision/apply in `src/adapters/assistantController.ts` to pass T063
- [X] T066 [US5] Create `src/ui/assistant/ConversationSwitcher.tsx` (dropdown in the region header: list with title + relative updated time, new, rename inline, delete with confirmation via `src/adapters/popups.ts`) and mount it in `src/ui/AssistantPanel.tsx`
...
- [X] T068 [P] [US6] Write failing contract tests `tests/contract/chat-context.test.ts` for `src/adapters/chatContext.ts` against a fake context: `chatMessages(n)` reads `getLiveAppContext().chat` (fresh per call), `characterCard()` via `getCharacterCardFields()` and `name2` (null without an open character), `persona()` from `name1` + `powerUserSettings.persona_description`, `activatedEntries()` caches the last `WORLD_INFO_ACTIVATED` payload (`world` + `uid`) and clears it on `CHAT_CHANGED`
- [X] T069 [US6] Implement `src/adapters/chatContext.ts` to pass T068 and replace the stub in `src/adapters/settingsStore.ts`
- [X] T070 [US6] Implement optional sources and discuss-mode handling in `src/core/assistant/context.ts` and `src/adapters/assistantController.ts` to pass T067
- [X] T071 [US6] Extend `src/ui/assistant/ContextMenu.tsx` with chat message count (0 = off), character card, persona and activated entries toggles, and update the context chip summary ("chat 10 · card · persona · activated")
...
- [X] T072 [P] Scale check: add a generated 300-entry workspace case to `tests/unit/assistant-context.test.ts` (whole-workspace scope, 8000-token limit → within budget, omitted list non-empty, < 200 ms) and a 50-operation reply case to `tests/unit/assistant-protocol.test.ts` (parse + validate < 50 ms)
- [X] T073 Typing responsiveness while streaming: throttle controller snapshot notifications to ≤ 10 per second during `receiving` and memoize `ConversationView` message rows so only the streaming row re-renders, in `src/adapters/assistantController.ts` and `src/ui/assistant/ConversationView.tsx`; verify editor typing during a stream in the live app (SC-007)
- [X] T074 Mobile layout pass: composer, proposal cards, menus and ProposalEditor inside the mobile bottom sheet (`src/ui/Sheet.tsx` mount in `src/ui/WorkspaceApp.tsx`), using existing sheet sizes; fix via shared primitives in `src/styles/wiw-theme.scss`
- [X] T075 Update `AGENTS.md`: project structure (`src/core/assistant/`, new adapters, `src/ui/assistant/`, tests/fixtures/support), Settings section (`assistant` block), assistant semantics section (request channel, streaming from preset, protocol, handles, destructive rule, undo, broken blocks + same-context regenerate, IndexedDB `WorldInfoWorkspace-assistant`), hooks (`assistant-applied`, `assistant-undone`), mark spec 005 as the current increment
- [X] T076 Bump version to `0.4.0` in `manifest.json` and `package.json`; run all four gates (`pnpm run typecheck`, `pnpm run lint`, `pnpm run test`, `pnpm run build`) and commit `dist/index.js`
- [X] T078 [US5] Keep the unsent composer text across closing/reopening the workspace (FR-002a): `src/adapters/composerDraft.ts` over `sessionStorage` (failures swallowed), used by `src/ui/assistant/Composer.tsx`; tests first in `tests/unit/assistant-draft.test.ts` (restore, clear on send/empty, blocked or missing storage); live scenario A25 in `quickstart.md`
- [X] T079 [US2] Items accepted one by one into a folder created by an earlier click (FR-012, live run 2026-09-16): `applyProposals` in `src/adapters/assistantApply.ts` resolves refs of creations applied before (not undone, still in the tree); failed proposals can be retried (`Retry` in `src/ui/assistant/ProposalCard.tsx`, not for destructive ones), the unresolved-folder reason names the folder proposal to apply first; test in `tests/integration/assistant-apply.test.ts`; live scenario A26 in `quickstart.md`
- [X] T080 [US2] Apply failed or undone proposals again (FR-012, live run 2026-09-17: after "undo" of the folder its entries could not land): `canApplyAgain` in `src/core/assistant/plan.ts` used by `applyProposals`, `confirmDestructive` and `ProposalCard` (`Retry` / `Apply again`, destructive ones via `Confirm …`); tests in `tests/integration/assistant-controller.test.ts`; live scenario A27 in `quickstart.md`
- [X] T081 [US2] One undo control per reply instead of one "Undo batch" button per accept click (FR-015, owner report 2026-09-17): `BatchBar` shows "Undo last" (its changes in the tooltip), "Undo all (N)" and one combined "reverted" badge; `undoAll` in `src/adapters/assistantController.ts` undoes newest first, an already undone batch is ignored; test in `tests/integration/assistant-controller.test.ts`; live scenario A28 in `quickstart.md`
- [X] T082 [US5] Composer input grows with its text up to 40dvh and shrinks after sending (owner request 2026-09-17, phones cannot drag the resize handle): `field-sizing: content` in `src/styles/wiw-theme.scss` like the app's `#send_textarea`, `scrollHeight` fallback in `src/ui/assistant/Composer.tsx` where unsupported; live scenario A29 in `quickstart.md`
- [X] T083 [US5] Enter on phones and an empty send (FR-004a, owner review 2026-09-22): `sendsOnEnter()` in `src/adapters/appApi.ts` follows the app's `shouldSendOnEnter` (Auto = never on phones/tablets; `pointer: coarse` fallback) in `src/ui/assistant/Composer.tsx`; `send('')` answers the last user message (`canAnswerLast`, `answerLast` in `src/adapters/assistantController.ts`); tests in `tests/integration/assistant-controller.test.ts`; live scenario A30
- [X] T084 [US5] Edit messages in place (FR-004b): `src/core/assistant/editReply.ts` (`editableText`, `editMessage`: re-parse, keep proposals of unchanged blocks via `OperationProposal.source`, keep applied/reverted ones whose block was removed, fresh ids, re-linked dependencies), `editMessage` in the controller, pencil tool + `MessageEditor` in `src/ui/assistant/ConversationView.tsx`; tests in `tests/unit/assistant-edit-reply.test.ts` and `tests/integration/assistant-controller.test.ts`; live scenario A31
- [X] T085 Splitters (owner review 2026-09-22): one `usePointerResize` hook + `<Splitter>` in `src/ui/Splitter.tsx` for the tree, assistant and preview splitters and the mobile sheet; drags write sizes to the DOM and commit to state on release (no re-render per pointermove); sizes persist in `settings.layout` (`src/core/state/layout.ts`, repaired field by field, not saved while a recovery is pending; `src/ui/layoutContext.ts` for the preview); tests in `tests/unit/layout-settings.test.ts`; live scenario A32
- [X] T086 Mobile sheet (owner report 2026-09-22): full size reaches the top of the World Info drawer (measured in `src/ui/Sheet.tsx`, the sheet stays inside the workspace container), glides to the snap point after release, `overscroll-behavior: contain` and the safe-area inset on `.wiw-sheet-content`; live scenario A33
- [X] T087 Preview renderer (owner review 2026-09-22): ordered and nested lists, pipe tables, strikethrough, multi-line/nested quotes and rules in `src/core/preview.ts`; links only for http(s)/mailto/anchors/relative paths (`javascript:` and other schemes stay text); shared `.wiw-markdown` block styles; showdown delegation reviewed and declined (image references, placeholders and reference markers need the own renderer); tests in `tests/unit/preview.test.ts`
- [X] T088 Diff (owner review 2026-09-22): Myers diff (`diffSequences`) replaces the LCS table in `src/core/diff/lineDiff.ts` and the word LCS in `src/core/assistant/rules.ts`; `diffWords` marks changed words; `src/ui/DiffView.tsx` renders one scroll container with rows holding both sides (wrapped lines stay level, no scroll sync needed), line numbers and +/− markers, and a unified view below 560 px width; tests in `tests/unit/line-diff.test.ts`; live scenario A34
- [X] T089 [US3] Remember the last context (FR-004c, owner request 2026-09-22): `updateConversationContext` also writes `AssistantSettings.defaultContext` (skipped while a recovery is pending); the "Use as default" button is replaced by a hint in `src/ui/assistant/ContextMenu.tsx`; tests in `tests/integration/assistant-controller.test.ts`; live scenario A35
- [X] T090 Preview review on the "Markdown demo" entry (owner request 2026-09-22): code-fence info strings are not code (`language-*` class), HTML comments are hidden, bare http(s)/mailto addresses become links, `#anchors` stay in the page, code spans are literal (no placeholders, no emphasis), markup inside link labels; tests in `tests/unit/preview.test.ts`; live scenario A36
- [X] T091 Preview edge cases (owner request 2026-09-22): line-based fences (``` and ~~~, unclosed = code to the end, inline triple backticks are code spans), CRLF, ATX h1–h6 with closing hashes and setext headings, backslash escapes, written entities, code spans of any backtick length, emphasis with flanking rules (`_`/`__`, `***`, no `** x **`), `==highlight==`, loose and task lists, escaped `\|` in tables, lazy quote continuation, Obsidian callouts and `[[wikilinks|alias]]`, links with titles and balanced parentheses, `<autolinks>`; hrefs always fully escaped (entities never decoded into a scheme), images only from http(s)/blob/`data:image`/relative sources; styles for task items, callouts, wikilinks and `mark` in `.wiw-markdown`; tests in `tests/unit/preview.test.ts`; live scenario A37
- [X] T092 [US4] Continue in place (live run 2026-09-22, FR-004): `continueReply` re-sends the reply's `requestMessages` + the reply so far (`continuationBase`: thinking and an unfinished trailing block dropped) + `CONTINUE_INSTRUCTION` (`src/core/assistant/prompts.ts`) instead of an assistant prefill, streams the rest into the SAME message (`appendContinuation`) and re-parses it through `editMessage`, so decisions and refs of the cut-off part stay; a failed continuation leaves the reply unchanged and returns the failure (toast in `src/ui/AssistantPanel.tsx`); tests in `tests/integration/assistant-controller.test.ts` and `tests/unit/assistant-edit-reply.test.ts`
- [X] T093 [US4] Thinking without an answer (live run 2026-09-22): `src/adapters/llmClient.ts` reports empty text with reasoning as the failure kind `thinking-only` ("…spent the whole response length on thinking… Raise \"Response length\"…"); test in `tests/contract/llm-client.test.ts`
- [X] T094 [US4] Operations written as function calls (live run 2026-09-22): `parseReply` reports a final reply without blocks that writes an operation as a call (`<|tool_call_start|>`, `create_entry(parent=…`) as one `malformed-block`, so "Ask to fix" is offered; tests in `tests/unit/assistant-protocol.test.ts`
- [X] T095 [US2] Delete confirmation text (live run 2026-09-22): an assistant deletion is undoable, so its confirmation says "\"Undo\" of this reply restores it." instead of "This cannot be undone." (`undoNote` option of `deleteNodes`/`describeDeletion`); test in `tests/integration/assistant-apply.test.ts`
- [X] T096 [US4] Echoed request context (live run 2026-09-22, A21): `parseReply` removes an echoed `<workspace>` block (closed or running to the end) from the prose, keeping the operation blocks around it; tests in `tests/unit/assistant-protocol.test.ts`
- [X] T077 Live validation: run quickstart A0–A37 on `openrouter free provider` and `openrouter small gemma free` (dev account, `playwright-cli`), including the parallel main-chat check A19 (FR-029), record results and the SC-005 valid-proposal rate in a "Validation runs" section of `specs/005-ai-lore-assistant/quickstart.md`; every discrepancy becomes a failing test before its fix

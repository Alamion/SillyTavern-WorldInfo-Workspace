...
- [ ] T077 Live validation: run quickstart A0–A26 on `openrouter free provider` and `openrouter small gemma free` (dev account, `playwright-cli`), including the parallel main-chat check A19 (FR-029), record results and the SC-005 valid-proposal rate in a "Validation runs" section of `specs/005-ai-lore-assistant/quickstart.md`; every discrepancy becomes a failing test before its fix
